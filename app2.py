...
def manage_data(filename):
    ...
    if request.method == 'GET':
        app.logger.info(f"Data read from file: {filename}")
        ...
        try:
            ...
            app.logger.info(f"New data added to {filename}: {new_data}")
            return jsonify(convert_nan_to_none(df.to_dict(orient='records'))), 200
        except Exception as e:
            app.logger.error(f"Error adding data to {filename}: {e}")
        ...
        try:
            ...
            app.logger.info(f"Data in {filename} updated: {update_data}")
            return jsonify(convert_nan_to_none(df.to_dict(orient='records'))), 200
        except Exception as e:
            app.logger.error(f"Error updating data in {filename}: {e}")
        ...
        try:
            ...
            app.logger.info(f"Data from {filename} deleted at indices: {delete_indices}")
            return jsonify(convert_nan_to_none(df.to_dict(orient='records'))), 200
        except Exception as e:
            app.logger.error(f"Error deleting data from {filename}: {e}")
...
def tsne_visualization():
    ...
    perplexity = min(30, max(5, len(words) - 1))  # Ensure perplexity is at least 5
    
    # Logging for debugging
    app.logger.debug(f"Number of words: {len(words)}")
    app.logger.debug(f"Vectors shape: {vectors.shape}")
    app.logger.debug(f"Perplexity: {perplexity}")
